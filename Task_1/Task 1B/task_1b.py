...
# 					file_num,img_file_path,final_point,no_cells_height,no_cells_width,img
...
def send_to_receive_from_server(sock, shortestPath):
	...
	sock.send(str.encode(str(file_num)+sent_data))
...
def find_new_path(recv_data, shortestPath):
	...
	#############  Add your Code here   ###############
	temp = tuple(recv_data[2:len(recv_data)-3])
	temp1=0
	temp2=0
	x=0
	if (obstacle_coord == "$"):
		...
		for i in range(len(temp)):
			if(temp[i]==','):
				...
				if(x==0):
					temp1 = temp1*10 + int(temp[i])
				else:
					temp2 = temp2*10 + int(temp[i])
		obstacle_coord = (temp1,temp2)
	...
	img[(obstacle_coord[0])*task_1a.CELL_SIZE : (obstacle_coord[0]+1)*task_1a.CELL_SIZE  ,  (obstacle_coord[1])*task_1a.CELL_SIZE : (obstacle_coord[1]+1)*task_1a.CELL_SIZE] = 0
